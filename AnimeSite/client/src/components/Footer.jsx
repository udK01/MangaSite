...
export default function Footer() {
  return (
    <section className="flex flex-col w-full mt-10 text-white">
      <div className="flex w-full h-auto justify-center items-center font-poppins bg-primary">
        {footer_text.map((footT, i) => (
          <p
            key={footT.id}
            className={`${
              i !== footT.length - 1 ? "ml-5" : ""
            } hover:bg-secondary hover:cursor-pointer p-2 transition-all duration-300 md:text-[16px] 2xs:text-[10px]`}
          >
            {footT.title}
          </p>
        ))}
